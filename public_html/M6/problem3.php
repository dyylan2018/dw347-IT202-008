...
function joinArrays($users, $activities) {
    ...
    // Start edits
    // Create a mapping of userId to user data for quick access
    $userMap = [];
    foreach ($users as $user) {
        $userMap[$user['userId']] = $user; // Store the user by userId
    }
    
    // Now join the users with their activities
    foreach ($activities as $activity) {
        if (isset($userMap[$activity['userId']])) {
            // Merge user information with the activity
            $joined[] = array_merge($userMap[$activity['userId']], $activity);
        }
    }
    // End edits
